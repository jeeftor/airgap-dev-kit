...
grep -q '^install.sh$' "$TMP_DIR/package-files.txt"
grep -q '^\.airgap-cli-only$' "$TMP_DIR/package-files.txt"
grep -q '^offline-packages/linux/tmux-3.4-static-x86_64$' "$TMP_DIR/package-files.txt"

if grep -q '^offline-packages/linux/wezterm.AppImage$' "$TMP_DIR/package-files.txt"; then
  echo "CLI-only package must not include WezTerm" >&2
  exit 1
fi

if grep -q '^fonts/' "$TMP_DIR/package-files.txt"; then
